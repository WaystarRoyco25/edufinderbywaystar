...
function buildGoogleAndRedditEvidencePrompt(
  ...
  return [
    ...
    "Prioritize detailed cases. Strongly prefer Reddit threads and comments where the applicant shares concrete specifics: unweighted and weighted GPA, SAT or ACT scores, class rank, course rigor (AP or IB counts), specific extracurriculars and awards, intended major, residency or demographic context, and the decision with its round. Skip thin posts that only state an outcome with no supporting numbers, and keep searching until you have well-documented cases.",
    ...
    "Capture every concrete detail the source states in applicantFacts as string key-value pairs. Use keys such as gpaUnweighted, gpaWeighted, sat, act, classRank, apIbCount, courseRigor, major, extracurriculars, awards, leadership, residency, and decision, and record quantitative values exactly as written. Richer applicantFacts makes the case far more useful; never return a near-empty applicantFacts when the source actually contains numbers and activities.",
    "Shape: {\"evidence\":[{\"sourceType\":\"google|reddit\",\"school\":\"...\",\"cycle\":\"...\",\"outcome\":\"admitted|waitlisted|denied|deferred|stat\",\"round\":\"ED|EA|RD|...\",\"program\":\"...\",\"applicantFacts\":{\"gpaUnweighted\":\"3.9\",\"sat\":\"1520\",\"classRank\":\"top 5%\",\"apIbCount\":\"10 APs\",\"extracurriculars\":\"robotics captain, founded coding club\",\"awards\":\"USACO Gold\"},\"quoteExcerpt\":\"short exact excerpt\",\"url\":\"https://...\",\"credibilityScore\":0.0}]}",
    `Target school: ${school.name}`,
    `Program/major: ${school.program || profile.major || "unspecified"}`,
    `Round: ${school.round || "unspecified"}`,
    `Applicant context: grade ${profile.education.grade}, citizenship ${profile.education.citizenship}, GPA ${profile.academics.gpaUnweighted}, testing SAT ${profile.testing.satTotal}, ACT ${profile.testing.actTotal}, current school ${profile.education.currentSchool}.`,
...
function buildDraftPrompt(input: DraftReportInput): string {
  return [
    ...
    "For every similarCases entry, write a substantive `summary` of two to four plain-English sentences. Describe who that applicant was using the concrete details in that evidence row's applicantFacts and quoteExcerpt: cite their actual numbers (GPA, SAT or ACT, class rank, AP or IB load) and name any standout extracurriculars or awards. Then give the admission outcome and round, and explain why the case is comparable to this student. The reader only sees this summary, never the link or the raw evidence, so it must be specific and self-contained. Do not write a vague summary that omits the numbers and activities when the evidence actually contains them.",
