...
import { captureReportPayPalOrder, CURRENCY, REPORT_PRICE } from "@/lib/paypal";
import { scrapUserReportData, userHasGeneratedReport } from "@/lib/report/purchase";

export const dynamic = "force-dynamic";

/**
 * Captures an approved PayPal order and records the Insight! Report
 * purchase, which grants the buyer one report credit.
 *
 * Body: { orderID: "<paypal order id>" }
 * Response: { ok: true }
 *
 * Idempotent: the `report_purchases.paypal_order_id` unique index means a
...
export async function POST(request: Request) {
  ...
  const capture = await captureReportPayPalOrder(orderId);
  if (!capture.ok) {
  ...
  if (capture.amountValue !== REPORT_PRICE || capture.amountCurrency !== CURRENCY) {
    return NextResponse.json(
      { error: "Payment amount did not match the report price" },
      { status: 409 },
    );
  }

  // Decided before the new row exists: a buyer who already has a report is
  // a returning buyer starting over.
  const startingOver = await userHasGeneratedReport(admin, user.id);

  const insert = await admin.from("report_purchases").insert({
    user_id: user.id,
  ...
  if (startingOver) {
    try {
      ...
      console.error("report scrap after purchase failed", err);
    }
  }
