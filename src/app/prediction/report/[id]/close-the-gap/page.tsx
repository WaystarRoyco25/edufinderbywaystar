...
export default async function CloseTheGapPage({ params }: PageProps) {
  ...
  return (
    <main className="mx-auto max-w-3xl px-4 py-8 md:py-10">
      {/* The PDF export bundles the full report ahead of the recommendations.
          It is hidden on screen and revealed only for the print dialog. */}
      {gap && reportJson && (
        <div className="hidden space-y-8 print:block">
          <ReportHeader title="The Insight! Report" subject={subject} />
      ...
      <div className="space-y-8 print:break-before-page">
        ...
        {!gap ? (
          <section className="break-inside-avoid rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <>
            {gapFocus?.rationale && (
              <section className="break-inside-avoid rounded-lg border border-blue-200 bg-blue-50 p-5 shadow-sm">
                ...
                <ul className="mt-4 space-y-3">
                  {gap.items.map((item) => (
                    <li
                      ...
                      className="break-inside-avoid rounded-lg border border-gray-200 bg-gray-50 p-4"
                    >
