...
export default async function DashboardPage() {
  const user = await requireDashboardUser("/dashboard/challenge");

  const admin = createSupabaseAdminClient();
  const [modulesResult, purchasesResult, ownership] = await Promise.all([
    admin
      .from("modules")
      .select(
  ...
  return (
    <main className="space-y-8">
      ...
      {!ownership.insight && <CrossSellCard service="insight" />}
    </main>
  );
}
